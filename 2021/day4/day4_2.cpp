...
bool isInWinnerBoards (vector<int> winnerBoards, int boardNumber) {
	for (int i = 0; i < winnerBoards.size (); i++) {
		if (winnerBoards [i] == boardNumber) {
			return true;
		}
	} 
	return false;
...
int main () {
	...
	vector<int> winnerBoards;

	for (int i = 0; i < bingoNumbers.size (); i++) {
		for (int j = 0; j < blocksCount; j++) {
			if (!isInWinnerBoards (winnerBoards, j)) {
				for (int k = 0; k < 25; k++) {
				...
				if (isWinner (blocksData [j])) {
					winnerBoards.push_back (j); 
					if (winnerBoards.size () == blocksCount) {
						cout << getResult (blocksData [j], stoi (bingoNumbers [i])) << endl;
						return 1;
					}
				}
			}
